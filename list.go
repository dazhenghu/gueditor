...
type ListInterface interface {
    GetFileList(fileList []*FileItem, start int, size int) error // 获取文件夹下的文件列表
...
获取资源列表
 */
func (l *List) GetFileList(fileList []*FileItem, start int, size int) error {
    ...
    if err == nil {
        ...
        filepath.Walk(path, func(fileName string, info os.FileInfo, err error) error {
            if fileName != "." && fileName != ".." {
                path2 := filepath.Join(path, fileName)
                if info.IsDir() {
